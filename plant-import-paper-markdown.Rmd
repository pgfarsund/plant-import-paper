...
ggplot(aes(x=factor(group, 
  ...
  annotate(geom = "text", 
           label = c("30.3 %", 
                     ...
                     "10.9 %",
                     "3.6 %"),
           x = rep(c(1,2,2.45,2), # we want "0.8 %" outside the actual bar, which makes this a bit tricky
                   times = c(4,1,1,4)), 
           y = c(0.84, 0.45, 0.18, 0.06, # plant percent labels
                 0.8, 0.6, 0.5, 0.26, 0.1, 0.02), # fungi percent labels
           size = 4, color = "black", fontface = 1) + 
  annotate(geom = "segment", x = 2.25, xend = 2.325, y = 0.60, yend = 0.60) + 
