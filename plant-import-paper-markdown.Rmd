...
p3 <- ggiNEXT(inext.res.p, type = 1, se = F) +
  ...
  scale_color_manual(values = c("darkolivegreen1", "darkolivegreen1",
                         "darkolivegreen4", "darkolivegreen4"), 
                     labels = c("C.1 - 20 pots", "C.1 - 10 pots", "C.2 - 20 pots", "C.2 - 10 pots")) +
...
psac$data[[1]]$fill <- c("darkolivegreen1", "darkolivegreen1",
                         "darkolivegreen4", "darkolivegreen4")
psac <- ggplot_gtable(psac); plot(psac)
library(ggplotify)
psac <- as.ggplot(psac); psac # pdf landscape 5 x 5
...
ggplot(obs.df, aes(x=interval, y=Observed, fill=host)) + 
  geom_boxplot()
...
fig3.a.p=obs.df %>% 
  ...
  theme(legend.title = element_blank(), 
        ...
        legend.key.width = unit(0.5, 'cm')); fig3.a.p
...
```{r}
twice1 = subset_samples(plants, interval == "double"); twice1 = prune_taxa(taxa_sums(twice1)>0,
                                                                         twice1); twice1
sort(sample_sums(twice1))
twice <- rarefy_even_depth(twice1, replace = F, trimOTUs = T, rngseed = 123, 
                           sample.size = 8629); twice

# p46 remains alone without its partner and needs to be removed: 
...
### ikke sikker på hvorfor jeg normaliserte fiphy her. poenget er å analysere
### fold increase i obs.spec.ric. fra 1 til 2 sekvenseringer, poenget forsvinner
### jo ved å normalisere prøvene etter at de er normalisert ... eller?
...
fig3.b.p=ggplot(fidf, aes(x=reorder(pot_id, -foldincrease), 
  ...
  coord_cartesian(ylim = c(0.9, 2.1)); fig3.b.p
...
fig3.c.p=ggplot(fidf, aes(x=obs_second, 
  ...
  coord_cartesian(ylim = c(0.9, 2.1)); fig3.c.p
  annotate(geom = "text", 
           x=8.8, 
           y=2.8, 
           label = paste0("r = ", 
...
library(ggplotify)
fsac <- as.ggplot(fsac); fsac # pdf landscape 5 x 5
...
anova(lm(Observed ~ interval, 
         data = obs.df))
anova(lm(Observed ~ host, 
         data = obs.df))
anova(lm(Observed ~ container, 
         data = obs.df))

anova(lm(Observed ~ interval * host, 
         data = obs.df))

mod <- lm(Observed ~ interval, data = obs.df); anova(mod)
...
```{r}
twice1 = subset_samples(fungi, interval == "double"); twice = prune_taxa(taxa_sums(twice)>0,
                                                                         twice); twice
sort(sample_sums(twice1))
twice <- rarefy_even_depth(twice1, 
                           replace = F, 
                           trimOTUs = T,
                           rngseed = 123, 
                           sample.size = min(sample_sums(twice1))); twice1; twice
...
fig3.b.f=ggplot(fidf, aes(x=reorder(pot_id, -foldincrease), 
  ...
  geom_hline(yintercept = c(1, 1.25, 1.5, 1.75, 
             ...
             linetype = 2, color = "grey50") +
  scale_y_continuous(breaks = seq(0, 3, 0.25), limits = c(0, 3)) + 
  coord_cartesian(ylim = c(0.9, 2.6)) +
  geom_col(width=0.8) +
  ...
  annotate(geom = "text", 
           ...
           hjust=0) +
  scale_y_continuous(breaks = seq(0, 3, 0.25), limits = c(0, 3)) + 
  coord_cartesian(ylim = c(0.9, 2.6)); fig3.b.f
...
fig3.c.f=ggplot(fidf, aes(x=obs_second, 
  ...
  labs(title = " ",
       subtitle = "H.") +
  scale_y_continuous(breaks = seq(0, 3, 0.25), limits = c(0, 3)) + 
  coord_cartesian(ylim = c(0.9, 2.6)); fig3.c.f
#
...
fungivenn <- ps_venn(ps = f.rar, 
                     ...
                     fill = list(fill=c("orange1",
                                        ...
                                        "orange4",
                                        "orange4"), 
                     alpha=0.75)); fungivenn

library(ggplotify)
ggfungivenn <- as.ggplot(fungivenn); ggfungivenn # cool
...
#
fmodbox <- ggarrange(fmodbox1, fmodbox2, nrow = 2)
modbox <- ggarrange(pmodbox, fmodbox, nrow = 1); modbox

venn <- ggarrange(ggplantvenn, NULL, ggfungivenn, 
                  nrow = 3, 
                  widths = c(1, 1, 1), 
                  heights = c(1, 0.1, 1)); venn # pdf 3x5



ggarrange(modbox, NULL, venn, nrow = 1, 
          heights = c(1, 1, 1),
          widths = c(3, 0.1, 1)) # pdf landscape 6 x 9
