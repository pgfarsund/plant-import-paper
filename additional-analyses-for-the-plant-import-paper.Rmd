...
fmodbox1 <- obs %>% 
  ...
  theme(legend.title = element_blank(),
        ...
        legend.box.background = element_rect(colour = "black", 
                                             size = 1),
        legend.margin = margin(3, 3, 3, 3)) +
  labs(title = "Contaminant fungal richness", subtitle = "Container 1 vs. Container 2"); fmodbox1
...
fmodbox2 <- obs %>% 
  ...
  theme(legend.title = element_blank(),
        ...
        legend.margin = margin(3, 3, 3, 3)) +
  ylab(" ") +
  labs(title = " ", subtitle = "Taxus vs. Thuja"); fmodbox2
...
library(mosaic)
mod <- lm(Observed ~ interval * host, 
          data = obs.df); summary(mod); tuk <- TukeyHSD(mod)

data.frame(tuk$`interval:host`) %>% 
  filter(p.adj < 0.051) %>% 
  print()
