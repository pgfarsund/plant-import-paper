...
gofstat(x, fitnames = c("normal"))

qqnorm(sqrt(obs$Observed), pch = 1, frame = FALSE)
qqline(sqrt(obs$Observed), col = "steelblue", lwd = 2)
...
p.posthoc = emmeans::emmeans(p.mod1, 
                    list(pairwise ~ container*host), 
                    adjust = "Tukey"); p.posthoc # two significant pairwise comparisons
...
NEED TO NORMALIZE FIRST:
  - normalizing all sams prior to merging = taking and seqing 2 per pot = expensive 
...
x<-plot_ordination(p.rar, 
  ...
  scale_color_manual(values = c("darkolivegreen1", 
                                "darkolivegreen4")); x
library(BiodiversityR)

p.rar@sam_data$cat <- paste0("Container ",
                             p.rar@sam_data$container, 
                             " ",
                             p.rar@sam_data$host); head(p.rar@sam_data$cat)

ordmod = vegan::metaMDS(p.rar@otu_table, distance = "bray", 
                        k = 2, try = 1,
                        utotransform=F, noshare=0.1, 
                        expand=TRUE, trace=1, plot=FALSE)

plot1 <- ordiplot(ordmod, choices=c(1,2))

sites.long1 <- sites.long(plot1, 
                          env.data=p.rar@sam_data)
...
p.ord <- ggplot() +
  ...
  geom_point(data=sites.long1, 
             aes(x=axis1, y=axis2, colour=cat, fill=cat, shape=cat),
             size=5,alpha=0.9) +
  scale_fill_manual(values = c("darkolivegreen1", "darkolivegreen4", "darkolivegreen1", "darkolivegreen4")) +
  ...
  ggforce::geom_mark_ellipse(data=sites.long1, 
                             aes(x=axis1, y=axis2, colour=cat, fill=NULL,
                          #fill=after_scale(alpha(colour, 0.2))
                          ), 
                      expand=0, size=0.2, show.legend=FALSE) 
  theme(legend.title = element_blank()); p.ord # pdf landscape 4x5
...
qqnorm(log(obs$Observed)/10, pch = 1, frame = FALSE)
qqline(log(obs$Observed)/10, col = "steelblue", lwd = 2)
...
glmm1 <- lme4::glmer(log(Observed)/10 ~ container + host + (1|pot_id), 
                     data = obs, 
                     family = beta_family()); summary(glmm1)
#
```
...
fungi.euler <- gridExtra::grid.arrange(fungicont, fungihost, 
                                       widths = c(1, 1), nrow = 1)
#
grid.arrange(cont1venn, cont2venn, 
             NULL, NULL, 
             fungicont, fungihost,
             heights = c(1, 0.2, 1),
             nrow = 3)
...
#####################################
taxa <- ps_venn(plants, group = "host", plot = F)
venn.df <- data.frame("Taxus"=taxa$Taxus, 
                      "Thuja"=taxa$Thuja,
                      "Both"=taxa$Taxus__Thuja)
ggVennDiagram::ggVennDiagram(taxa)

# use ggVennDiagram

taxus <- subset_samples(plants)
