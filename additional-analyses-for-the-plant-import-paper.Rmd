...
cont2 = subset_samples(p.rar, container == "2"); cont2
...
ggiNEXT(conts.out, type = 1, se = TRUE) +
  theme_classic()
...
##### container comparison fungi
  pnext = data.frame(t(fungi@otu_table)) %>% 
  mutate_if(is.numeric, ~1 *(. !=0)) %>% 
  list()

cont1 = subset_samples(f.rar, container == "1") 
cont1inext = data.frame(t(cont1@otu_table)) %>% 
  mutate_if(is.numeric, ~1 *(. !=0))

cont2 = subset_samples(f.rar, container == "2"); cont2
cont2inext = data.frame(t(cont2@otu_table)) %>% 
  mutate_if(is.numeric, ~1 *(. !=0))
